<template>
  <div class="form-container">
    <form class="main-form" @submit.prevent="editDivision">
...
onMounted(async () => {
  try {
    const response = await http.get('/divisions/'+divisionId, {
      headers: {
        Authorization: `Bearer ${auth.getToken}`
      }
    })
    division.name = response.data.name
    division.visibleName = response.data.visibleName
    division.color = response.data.color
  } catch (err) {
...
async function editDivision(){
    try{
      await http.put('/divisions/'+divisionId, division, {
      headers: {
        Authorization: `Bearer ${auth.getToken}`
      }});
